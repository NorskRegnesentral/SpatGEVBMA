...
Rprof()
a <- spatial.gev.bma(Y.list, X, S, 10^5,print.every=2000)
Rprof(NULL)
...
### For comparison with the old version of SpatialGEVBMA
library(SpatialGEVBMA,lib.loc="/nr/samba/user/jullum/Rlibstest/old_packages")

set.seed(123)

Rprof()
b <- spatial.gev.bma(Y.list, X, S, 10^5,print.every=2000)
Rprof(NULL)
...
pdf(file="comp_with_nostarred_SpatialGEVBMA_10.pdf")
