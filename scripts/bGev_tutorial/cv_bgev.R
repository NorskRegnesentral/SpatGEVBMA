...
for(thisyear in 1980:2020){
  ...
  formula2 = inla.mdata(y,cbind(lon,lat),cbind(lon,lat)) ~ -1 + intercept + wetterdays_JJA_static_clim+precip_JJA_static_clim+temp_JJA_static_clim+masl+lon+lat
  ...
  formula4 = inla.mdata(y,amax2[,.(lon,lat)] ,amax2[,.(lon,lat)]) ~ -1 + beta0_intercept + wetterdays_JJA_static_clim+precip_JJA_static_clim+temp_JJA_static_clim + masl+lon+lat+f(field,model=spde)
  ...
  stackinfo4<- inla.stack(data=list(y=amax2$y),
                          ...
                          effects=list(c(s.index,beta0_intercept=1),
                                       c(list(wetterdays_JJA_static_clim=amax2$wetterdays_JJA_static_clim),
                                         list(precip_JJA_static_clim=amax2$precip_JJA_static_clim),list(temp_JJA_static_clim=amax2$temp_JJA_static_clim),
                                         list(masl=amax2$masl),
                                         list(lon=amax2$lon),list(lat=amax2$lat))), tag="field")
  ...
  xi3=inlares3$summary.hyperpar$mean[2]+inlares3$summary.hyperpar$mean[5]*amax2$lon+inlares3$summary.hyperpar$mean[6]*amax2$lat
  ...
  inlaparam=cbind(matrix(rep(inlares1$summary.fixed$`0.5quant`,each=dim(res1)[1]),byrow=FALSE,nrow=dim(res1)[1]),matrix(rep(c(inlares1$summary.hyperpar$`0.5quant`,NA,NA),each=dim(res1)[1]),byrow=FALSE,nrow=dim(res1)[1]))
  colnames(inlaparam)=c(paste0("beta0.5_",c("intercept","wetterdays_JJA","precip_JJA","temp_JJA","masl","lon","lat")),paste0("quant0.5_",c("spread","tail","beta1_spread","beta2_spread","beta3_tail","beta4_tail","range","sig")))
  
  inlaparam_u=cbind(matrix(rep(inlares1$summary.fixed$`0.975quant`,each=dim(res1)[1]),byrow=FALSE,nrow=dim(res1)[1]),matrix(rep(c(inlares1$summary.hyperpar$`0.975quant`,NA,NA),each=dim(res1)[1]),byrow=FALSE,nrow=dim(res1)[1]))
  colnames(inlaparam_u)=c(paste0("beta0.975_",c("intercept","wetterdays_JJA","precip_JJA","temp_JJA","masl","lon","lat")),paste0("quant0.975_",c("spread","tail","beta1_spread","beta2_spread","beta3_tail","beta4_tail","range","sig")))
  ...
  colnames(inlaparam_l)=c(paste0("beta0.025_",c("intercept","wetterdays_JJA","precip_JJA","temp_JJA","masl","lon","lat")),paste0("quant0.025_",c("spread","tail","beta1_spread","beta2_spread","beta3_tail","beta4_tail","range","sig")))
...
fwrite(reslist,"/nr/samba/user/roksvag/GitRepo/SpatGEVBMA/scripts/bGev_tutorial/inlares.csv")
